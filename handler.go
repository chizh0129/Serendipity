...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
    ...
    user := r.Context().Value("user") //raw user string
    claims := user.(*jwt.Token).Claims
    username := claims.(jwt.MapClaims)["username"]

    //variable = user.(*jwt.Token) => cast, it is called type assertion in go
...
func searchHandler(w http.ResponseWriter, r *http.Request) {//request is struct, can be instanizited, we use pointer
                                                            //requestWriter is interface, cannot be instianizatied
...
func signupHandler(w http.ResponseWriter, r *http.Request) {
    ...
    //sanity check regex, match password string
